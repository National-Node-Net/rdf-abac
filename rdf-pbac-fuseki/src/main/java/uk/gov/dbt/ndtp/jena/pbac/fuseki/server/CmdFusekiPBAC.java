...
package uk.gov.dbt.ndtp.jena.pbac.fuseki.server;
...
import uk.gov.dbt.ndtp.jena.pbac.fuseki.FMod_PBAC;
...
/**
 * Run Jena Fuseki with rdf-pbac available.
 * <p>
 * <b>For development</b>
 * <p>
 * User is given by "Bearer user:NAME".
 */
public class CmdFusekiPBAC {

    static {
        JenaSystem.init();
    }

    private static Symbol debugPBAC = Symbol.create("pbac:debug");

    public static void main(String ...args) {
        FusekiServer server = build(args).build();

        boolean bDebugPBAC = Fuseki.getContext().isTrue(debugPBAC);

        if ( bDebugPBAC ) {
            FmtLog.info(SysPBAC.SYSTEM_LOG, "Setting CxtPBAC.systemTrace");
            CxtPBAC.systemTrace(Track.TRACE);
        }
    ...
    public static FusekiServer.Builder build(String ...args) {
        ...
        // Must be after FMod_PBAC which can change plain operations into auth operations.
        FusekiModules modules = FusekiModules.create( new FMod_PBAC(), bearerAuthFilter);
