...
public interface LabelsStore { // extends Transactional {
    ...
    /**
     * The distinct labels held by this store. Excludes the dataset default label, which is not
     * stored; see {@link Labels#vocabulary(LabelsStore, String)} for the complete vocabulary.
     * Implementations that cannot enumerate throw rather than return an empty set, which would be
     * indistinguishable from a dataset holding no labels.
     *
